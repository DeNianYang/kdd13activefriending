...
inline int visitf(int v, int r) {
	return f.find(key2(v,r)) != f.end();
}
...
void findFriends(long long srcId) {
	...
	FILE * fout = fopen(input, "w");
	long long A, B;
	while ( fscanf(fin, "%lld%lld%*s", &A, &B) != EOF ) {
...
int ** bucket;
double F(int v, int r);

void permutation(int v, int r, int remain, int t) {
	if ( t == link[v].size() ) {
		if ( remain == 0 ) {
			double w = 1;
			for ( int i = 0 ; i < link[v].size() ; i++ ) {
				w *= (1-F(link[v][i].B, bucket[v][i])*link[v][i].w);
			}
			w = 1-w;
			if ( getf(v,r) < w )
				setf(v,r,w);
	...
	int B = link[v][t].B;
	for ( int i = 0 ; i <= nv[B] && i <= remain ; i++ ) {
		bucket[v][t] = i;
		permutation(v, r, remain-i, t+1);
	}
}
...
double F(int v, int r) {
	...
	if ( visitf(v, r) )
		return getf(v, r);
	bucket[v] = (int*)malloc(sizeof(int)*link[v].size());
	fill(bucket[v], bucket[v]+link[v].size(), 0);
	permutation(v,r,r-1,0);
	free(bucket[v]);
	bucket[v] = NULL;
	return getf(v,r);
}

// a 30 1 1 0.00001 0 7
