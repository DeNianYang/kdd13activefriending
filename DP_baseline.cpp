...
int main(int args, char* argv[])
{
	...
	sscanf(argv[2], "%d", &NT);
	long long * tarId = (long long*)malloc(sizeof(long long)*NT);
	for ( int i = 0 ; i < NT ; i++ ) {
	...
	sort(tarId, tarId+NT);	// Make the order unique
	...
	for ( int i = 0 ; i < NT ; i++ ) {
		...
		if ( !fin )
			constructTree(tarId[i], threshold);
		else
			fclose(fin);
	}
	...
	for ( int t = 0 ; t < NT ; t++ ) {
		...
		sprintf(input, "../tree/%lld-%.8lf.txt", tarId[t], threshold);
		fin = fopen(input, "r");
		...
		//backTrace(T,R);
		int n = (nv[T] < R) ? nv[T] : R;
		maxP = (getf(T,n) < maxP) ? maxP : getf(T,n);
		/*for ( int i = 1 ; i <= N ; i++ ) {
			if ( visit[i] )
	...
	printf("%lf %lf\n", maxP, ((double)(clock()-start))/CLOCKS_PER_SEC);
	return 0;
}
